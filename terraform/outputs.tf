...
output "vless_link" {
  ...
  value       = "vless://${random_uuid.vless_uuid.result}@${oci_core_public_ip.mtproxy_reserved_ip.ip_address}:${var.vless_port}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${var.vless_dest_domain}&fp=chrome&pbk=${data.external.vless_reality_keys.result.public_key}&sid=&type=tcp#OCI-VLESS-Reality"
  sensitive   = true
}

output "hiddify_import_link" {
  description = "Hiddify deep link - tap on phone to import VLESS config directly into Hiddify"
  value       = "hiddify://import/vless://${random_uuid.vless_uuid.result}@${oci_core_public_ip.mtproxy_reserved_ip.ip_address}:${var.vless_port}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${var.vless_dest_domain}&fp=chrome&pbk=${data.external.vless_reality_keys.result.public_key}&sid=&type=tcp#OCI-VLESS-Reality"
  sensitive   = true
}
