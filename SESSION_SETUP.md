...
Three containers run on the same instance, all deployed automatically via cloud-init:
...
│   ├── network.tf            # VCN, subnet, security list (ports 443, 9443, 8443)
│   ├── ocir.tf               # OCIR repository and auth token
│   ├── outputs.tf            # Terraform outputs (proxy links, VLESS + deep links)
