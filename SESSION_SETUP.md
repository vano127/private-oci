# Session Setup Guide

This document captures all the tools and setup used to deploy VLESS + Reality proxy on OCI.

**IMPORTANT:** All infrastructure changes must be applied through Terraform for reproducibility. Manual commands via bastion are for testing only.

## Prerequisites

- Terraform installed
- kubectl configured with access to EKS cluster
- OCI CLI configured (for checking available images)
- SSH key pair (`~/.ssh/id_ed25519`)
- Zscaler certificate for HTTPS fetches (corporate network)

### Zscaler Certificate

For HTTPS requests through corporate proxy, use the zscaler certificate:

```bash
# Curl with zscaler cert
curl --cacert /Users/kmvr200/IdeaProjects/personal-oci/zscaler.crt https://example.com

# Or set environment variable
export CURL_CA_BUNDLE=/Users/kmvr200/IdeaProjects/personal-oci/zscaler.crt
export REQUESTS_CA_BUNDLE=/Users/kmvr200/IdeaProjects/personal-oci/zscaler.crt
```

### Required Environment Variables for Terraform

OCI Object Storage S3-compatible backend requires these env vars for all `terraform` commands:

```bash
export AWS_CA_BUNDLE="/Users/kmvr200/IdeaProjects/personal-oci/zscaler.crt"
export AWS_REQUEST_CHECKSUM_CALCULATION=when_required
export AWS_RESPONSE_CHECKSUM_VALIDATION=when_required
```

`AWS_REQUEST_CHECKSUM_CALCULATION=when_required` is needed because OCI Object Storage does not support AWS chunked transfer encoding. Without it, state uploads fail with `501 NotImplemented`.

## Infrastructure Overview

- **Cloud Provider**: Oracle Cloud Infrastructure (OCI) Free Tier
- **Instance Shape**: VM.Standard.E2.1.Micro (1 OCPU, 1GB RAM)
- **OS**: Ubuntu 22.04 Minimal (chosen for low memory footprint ~200MB vs Oracle Linux ~400MB)
- **Region**: eu-frankfurt-1 (Frankfurt - Free Tier home region)
- **Static IP**: Reserved public IP (persists across instance recreates)
- **Terraform State**: Remote, OCI Object Storage via S3-compatible backend (bucket: `terraform-state`)

## Current Configuration

Two containers run on the instance, deployed automatically via cloud-init:

| Container | Image | Port | Purpose |
|-----------|-------|------|---------|
| xray-vless | teddysun/xray | 443 | VLESS + Reality server (TZ=UTC) |
| nginx-sub | nginx:alpine | 8080 | Subscription server (Hiddify, v2raytun) |

### VLESS + Reality Server
- **Port**: 443 (configurable via `vless_port`)
- **Protocol**: VLESS with Reality TLS
- **Container**: xray-vless (TZ=UTC for correct log timestamps)
- **Config**: /home/ubuntu/xray-config.json
- **Keys**: x25519 keypair generated by Terraform (no runtime generation)
- **SNI Domain**: www.oracle.com (configurable via `vless_dest_domain`)
- **Short ID**: `0123456789abcdef` (no empty short ID — prevents unauthenticated probes)

### Subscription Server
- **Port**: 8080
- **Container**: nginx-sub
- **Config**: /home/ubuntu/nginx-sub.conf
- **Endpoint**: `GET /sub/<token>` — returns base64-encoded VLESS link
- **Auth**: Secret random token in URL path (32 hex chars), 404 for all other paths
- **Headers**: `profile-title`, `profile-update-interval` (1 hour auto-refresh)
- **No routing rules** — client app handles routing
- **Compatible with**: Hiddify, v2raytun, v2rayNG, Streisand

### Get VLESS + Reality Link

Reality x25519 keys are generated by Terraform at plan time. The complete VLESS link is available directly as an output — no manual key retrieval needed.

```bash
cd /Users/kmvr200/IdeaProjects/personal-oci/terraform

# Complete VLESS link (works in v2rayNG, Hiddify, Streisand, v2raytun)
terraform output -raw vless_link

# Hiddify deep link (one-tap import on phone)
terraform output -raw hiddify_import_link

# v2raytun deep link (one-tap import on phone)
terraform output -raw v2raytun_import_link

# Reality public key only
terraform output -raw vless_public_key
```

### Get Subscription Link

The subscription URL returns the VLESS link. Clients auto-refresh hourly, so IP changes after instance recreates are picked up automatically.

```bash
cd /Users/kmvr200/IdeaProjects/personal-oci/terraform

# Subscription URL (share with users or use in QR code)
terraform output -raw subscription_url

# v2raytun deep link for subscription import
terraform output -raw v2raytun_subscription_link

# Hiddify subscription deep link
# hiddify://import/<subscription_url>

# Generate QR code for subscription
python3 scripts/generate-qr.py
```

**Mobile Clients for VLESS + Reality:**
- **Android**: Hiddify (free, supports TLS fragment), v2raytun (Google Play), v2rayNG (free, open-source)
- **iOS**: Streisand (free), Shadowrocket ($2.99)

---

## DPI Evasion

### Server-Side (configured in Terraform)
- **Port 443**: Standard HTTPS port, blends with normal traffic
- **SNI www.oracle.com**: Hosted on OCI (same cloud provider, no IP-SNI mismatch), not commonly used by proxy tools
- **No empty short ID**: Prevents unauthenticated DPI probes from getting valid TLS responses
- **Reality TLS**: DPI sees traffic identical to legitimate HTTPS to oracle.com

### Client-Side (configured in app settings)
Users experiencing high latency from Russia should enable these in their client app:

1. **TLS Fragment** (most effective): packets `tlshello`, length `100-200`, interval `10-20`
2. **MUX/Multiplex**: concurrency `8` (reduces handshakes DPI can inspect)
3. **TLS fingerprint**: try `firefox`, `safari`, or `random` instead of `chrome`

Hiddify has built-in TLS fragment support: Settings → Config Options → TLS Fragment.

### If Still Blocked
1. Force a new IP: `terraform taint oci_core_public_ip.mtproxy_reserved_ip && terraform apply`
2. Try a different SNI domain (update `vless_dest_domain` in tfvars, recreate instance)
3. Russian DPI (TSPU) may flag entire cloud provider IP ranges — consider residential proxy if OCI range is blocked

---

## Terraform Commands

All commands should be run from `/Users/kmvr200/IdeaProjects/personal-oci/terraform`

### Initialize
```bash
cd /Users/kmvr200/IdeaProjects/personal-oci/terraform
terraform init -backend-config=backend.conf
```

### Deploy Infrastructure
```bash
cd /Users/kmvr200/IdeaProjects/personal-oci/terraform
terraform apply
```

### Change Proxy Configuration

To change ports or SNI domain, update `terraform.tfvars`:

```hcl
# VLESS + Reality
vless_port        = 443
vless_dest_domain = "www.oracle.com"
```

Then recreate the instance to apply changes:

```bash
cd /Users/kmvr200/IdeaProjects/personal-oci/terraform
terraform apply -replace=oci_core_instance.mtproxy
```

### Force New IP

Taint the reserved IP to get a fresh public address:

```bash
cd /Users/kmvr200/IdeaProjects/personal-oci/terraform
terraform taint oci_core_public_ip.mtproxy_reserved_ip
terraform apply
```

**Note:** If only the IP changes (no instance recreate), the subscription server must be restarted to pick up the new IP. Recreate the instance or restart nginx-sub manually via bastion.

### Get Outputs
```bash
cd /Users/kmvr200/IdeaProjects/personal-oci/terraform

# Get all outputs
terraform output

# Get specific outputs
terraform output instance_public_ip
terraform output -raw vless_link
terraform output -raw hiddify_import_link
terraform output -raw v2raytun_import_link
terraform output -raw subscription_url
terraform output -raw v2raytun_subscription_link
```

### Destroy Infrastructure
```bash
cd /Users/kmvr200/IdeaProjects/personal-oci/terraform
terraform destroy
```

---

## Bastion Script Usage (Testing Only)

Use the bastion script for testing and debugging. For permanent changes, update Terraform instead.

**IMPORTANT:** Direct SSH from local machine does not work (corporate network). Always use the bastion pod.

```bash
# Setup bastion pod
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh setup

# Check status
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh status

# Execute command on OCI instance (testing only)
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh exec "sudo docker ps"
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh exec "sudo docker logs xray-vless"

# Interactive SSH
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh ssh

# Cleanup
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh cleanup
```

### Host key changed after instance recreate
```bash
# Clear known_hosts on bastion pod directly via kubectl
kubectl --context "arn:aws:eks:eu-west-1:747626100725:cluster/az-img-dev-kfv2-eks" exec bastion-pod -- sh -c "ssh-keygen -R <instance_ip> 2>/dev/null"
```

---

## Monitoring

### Check Proxy Status
```bash
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh exec "sudo docker ps"
```

### Check VLESS Connections
```bash
# Active connections on VLESS port
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh exec "ss -tn state established '( sport = :443 )' | tail -n +2 | wc -l"

# Xray logs
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh exec "sudo docker logs xray-vless --tail 50"
```

### Check Subscription Server
```bash
# Test subscription endpoint from bastion
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh exec "curl -s http://localhost:8080/sub/<token>"
```

---

## OCI Free Tier Limits

| Resource | Limit | Current Usage |
|----------|-------|---------------|
| AMD Compute (VM.Standard.E2.1.Micro) | 2 instances | 1 |
| Boot Volume | 200 GB | 47 GB |
| Reserved Public IP | Included | 1 |
| Outbound Data | 10 TB/month | ~minimal |

**Free tier has no time limit** - resources remain free indefinitely.

---

## File Structure

```
/Users/kmvr200/IdeaProjects/personal-oci/
├── terraform/
│   ├── backend.conf          # S3 backend credentials (not in git)
│   ├── cloud-init.yaml       # Instance initialization (Docker + containers)
│   ├── compute.tf            # Instance, IPs, and VLESS UUID generation
│   ├── network.tf            # VCN, subnet, security list (ports 22, 443, 8080)
│   ├── outputs.tf            # Terraform outputs (VLESS links, subscription)
│   ├── providers.tf          # Provider config + S3 remote backend
│   ├── variables.tf          # Variable definitions
│   ├── terraform.tfvars      # Variable values (not in git)
│   └── scripts/
│       └── derive-x25519-pubkey.py  # x25519 key derivation for Terraform external data source
├── scripts/
│   ├── bastion-setup.sh      # Helper script for SSH via K8s bastion
│   └── generate-qr.py        # QR code generator for subscription/VLESS links
├── zscaler.crt               # Zscaler certificate for HTTPS fetches
└── SESSION_SETUP.md          # This file
```

### Instance File Layout

```
/home/ubuntu/
├── xray-config.json          # VLESS + Reality Xray config (generated by cloud-init)
└── nginx-sub.conf            # Nginx subscription endpoint config (generated by cloud-init)
```

---

## Troubleshooting

### Cloud-init not completing
```bash
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh exec "sudo cat /var/log/cloud-init-output.log"
```

### Docker not starting
```bash
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh exec "sudo systemctl status docker"
```

### VLESS + Reality issues
```bash
# Check Xray container
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh exec "sudo docker logs xray-vless"

# Check iptables
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh exec "sudo iptables -L INPUT -n --line-numbers"

# Test port connectivity from bastion
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh exec "nc -zv -w 5 <ip> 443"

# Restart Xray
/Users/kmvr200/IdeaProjects/personal-oci/scripts/bastion-setup.sh exec "sudo docker restart xray-vless"
```

### VLESS client not connecting
1. Verify port is open: test from bastion with `nc -zv -w 5 <ip> 443`
2. Check OCI security list has the port rule (terraform apply if needed)
3. Check iptables on instance: `sudo iptables -L INPUT -n --line-numbers`
4. Verify all VLESS settings match exactly (UUID, public key, SNI, short ID, flow)
5. v2rayNG is not in Russian Play Store - download APK from GitHub: `https://github.com/2dust/v2rayNG/releases`
6. v2raytun is available on Google Play: `https://play.google.com/store/apps/details?id=com.v2raytun.android`
7. Alternative Android clients: Hiddify (recommended — has TLS fragment), NekoBox

---

## VLESS + Reality Overview

VLESS + Reality is an advanced censorship evasion protocol:

- **VLESS**: Lightweight proxy protocol (lighter than VMess)
- **Reality**: TLS fingerprint masquerading technology

### How Reality Works

Unlike traditional TLS proxies that use fake certificates, Reality:
1. Uses the actual TLS certificate of a legitimate website (e.g., oracle.com)
2. Performs server-side TLS handshake impersonation
3. DPI sees traffic that looks identical to legitimate HTTPS

### Current Configuration

| Setting | Value |
|---------|-------|
| Port | 443 |
| SNI/Dest | www.oracle.com |
| Flow | xtls-rprx-vision |
| Short IDs | "0123456789abcdef" |

---

## Important Notes

### Architecture
- OCI Free Tier instances use **AMD64** architecture
- When building Docker images on Apple Silicon (M1/M2/M3), always use `--platform linux/amd64`

### Port Allocation

| Port | Service | Protocol |
|------|---------|----------|
| 443 | VLESS + Reality | VLESS over Reality TLS |
| 8080 | Subscription server | HTTP (nginx, secret token path) |

### Security
- Share VLESS connection URLs and deep links only via private/encrypted channels
- Reality x25519 keys are generated by Terraform and stored in state (sensitive)
- The private key is injected into cloud-init and passed to the Xray container
- Subscription endpoint is secured with a random 32-char hex token in the URL path
- Subscription URL is HTTP (not HTTPS) — the token prevents URL guessing but content is unencrypted
